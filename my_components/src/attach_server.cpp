...
AttachServer::AttachServer(const rclcpp::NodeOptions &options)
    ...
    server_ = this->create_service<GoToLoading>(
        "/approach_shelf",
        std::bind(&AttachServer::handle_request, this, std::placeholders::_1, std::placeholders::_2)
    );

    // Initilaize static TF
    ...
    vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(cmd_topic, 10);
    
    // Initialize elevator publisher
    elevator_up_pub_ = this->create_publisher<std_msgs::msg::String>("/elevator_up", 10);

    RCLCPP_INFO(this->get_logger(), "AttachServer constructed and ready...");
}
...
// Find shelf 
//------------
std::vector<int> AttachServer::find_reflective_indices(const sensor_msgs::msg::LaserScan::SharedPtr scan) {
...
// Find shelf legs 
//------------------
std::vector<std::vector<int>> AttachServer::find_shelf_leg_cluster(const std::vector<int> &laser_index) {
...
//--------------------------------
std::vector<int> AttachServer::average_index(const std::vector<std::vector<int>> &clusters) {
...
//---------------------------------
bool AttachServer::index_to_xy(const sensor_msgs::msg::LaserScan &scan, int i, std::pair<double, double> &out_xy){
...
//------------------------------
bool AttachServer::laserXY_to_odom(double lx, double ly, const std::string &laser_frame, double &ox, double &oy){
...
// Set static TF
//---------------
void AttachServer::publish_cart_frame_static_once(double mx, double my, 
...
// Drive to TF frame
//-------------------
bool AttachServer::drive_to_cart_frame(const std::string &child_frame, double xy_tol, double max_time_sec) {
    ...
    while (rclcpp::ok() && (this->now() - t0).seconds() < max_time_sec ) {
        ...
        if (distance < xy_tol || (dx < 0.12 && std::abs(dy) < 0.05)) {
...
// Move foward 30cm
//------------------
void AttachServer::creep_forward(double distance_m, double speed_mps, double max_time_sec) {
    // Safty gaurd
    if (distance_m <= 0.0 || speed_mps <= 0.0) return;

    rclcpp::Rate rate(20.0);
    const auto t0 = this->now();
    
    // 1. Snapshot start pose in odom
    geometry_msgs::msg::TransformStamped T0;
    try {
        T0 = tf_buffer_->lookupTransform("odom", "robot_base_link", tf2::TimePointZero);        
    } catch (const tf2::TransformException &ex) {
        RCLCPP_WARN(this->get_logger(), "creep_forward: cant' read start pose %s", ex.what());
        return;
    }
    const double x0 = T0.transform.translation.x;
    const double y0 = T0.transform.translation.y;
    
    // 2. Drive forward and measure distance in odom
    while (rclcpp::ok() && (this->now() - t0).seconds() < max_time_sec) {
        rate.sleep(); // 20hz 

        geometry_msgs::msg::TransformStamped T;
        try {
            T = tf_buffer_->lookupTransform("odom", "robot_base_link", tf2::TimePointZero);
        } catch (const tf2::TransformException &ex) {
            RCLCPP_WARN(this->get_logger(), "creep_forward: odom lookup failed %s", ex.what());
            continue;
        }

        const double dx = T.transform.translation.x - x0;
        const double dy = T.transform.translation.y - y0;
...
// Shutdown 
//----------
void AttachServer::graceful_shutdown_after(std::chrono::milliseconds delay) {
...
// Service 
//----------
void AttachServer::handle_request(const std::shared_ptr<GoToLoading::Request> request,
    ...
    // 7. Move to shelf
    if (!drive_to_cart_frame(child, xy_tol, max_timer_sec)) {
    ...
    // 8. Move 30cm more 
    creep_forward(0.30, speed_mps, max_timer_sec);
